...
async function twelveDataRequest<T>(path: string, params: Record<string, string>) {
  ...
  if (!apiKey) throw new MarketDataError("TWELVE_DATA_API_KEY is not configured", 503);
  ...
  try { return await value; } catch (error) { requestCache.delete(cacheKey); throw error; }
}
...
export class TwelveDataProvider implements MarketDataProvider {
  async getQuotes(symbols: readonly PortfolioSymbol[]): Promise<QuoteSnapshot[]> {
    const snapshots = await Promise.all(symbols.map(async (symbol) => {
      ...
      return { symbol, price: latest.close, change: previous ? latest.close - previous.close : null, changePercent: previous?.close ? ((latest.close - previous.close) / previous.close) * 100 : null, currency: "USD", asOf: latest.time };
    }));
  ...
  async getHistory(symbol: PortfolioSymbol, outputSize = 120): Promise<OhlcvBar[]> {
    const series = await twelveDataRequest<TwelveDataTimeSeries>("/time_series", { symbol, interval: "1day", outputsize: String(outputSize), order: "ASC" });
    return (series.values || []).map((bar) => ({ time: bar.datetime, open: Number(bar.open), high: Number(bar.high), low: Number(bar.low), close: Number(bar.close), volume: Number(bar.volume || 0) }));
  }
}
