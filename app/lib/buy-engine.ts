...
function inZone(price: number, zone: ZoneDefinition) {
  ...
  const lower = zone.min ?? Number.NEGATIVE_INFINITY;
  const upper = zone.max ?? Number.POSITIVE_INFINITY;
  return zone.zone === "B"
...
function getDynamicLevels(price: number, indicators: IndicatorSnapshot) {
  const step = Math.abs(indicators.atr14 ?? price * 0.03);
  const minimumStep = Math.max(step * 0.5, Math.abs(price) * 0.01, 0.01);
  const first = Math.min(price - minimumStep, indicators.ema20 ?? price - minimumStep);
  const second = Math.min(first - minimumStep, indicators.support20 ?? first - step);
  const third = Math.min(second - minimumStep, indicators.support60 ?? second - step);
  return {
...
export function calculateBuyEngine(symbol: PortfolioSymbol, price: number, indicators: IndicatorSnapshot, macro: MacroSnapshot[]) {
  ...
  const maximum = rules
    .filter((rule) => rule.signal !== "Unavailable")
    .reduce((total, rule) => total + rule.maximum, 0);
  const normalizedScore = maximum ? Math.round((score / maximum) * 100) : 0;
  const dataAvailable = indicators.rsi14 !== null && indicators.ema20 !== null;
  const action: BuyAction = !dataAvailable || currentZone === "A" ? "WAIT" : normalizedScore >= 60 ? "ACCUMULATE" : normalizedScore >= 30 ? "WATCH" : "WAIT";
  ...
  const entryLevels = [
    { label: "Dynamic entry", price: round(firstEntry), allocation: "20%" },
    { label: "20-day low support", price: round(secondEntry), allocation: "35%" },
    { label: "60-day low support", price: round(thirdEntry), allocation: "30%" },
  ];
  ...
  if (indicators.support20 !== null) reasons.push(`20-day low support is ${indicators.support20.toFixed(2)}`);
  if (!reasons.length) reasons.push("No accumulation rule is currently triggered");
  return { symbol, price, currentZone, score, maximum, confidence: maximum, normalizedScore, action, dataAvailable, preferredEntry, entryLevels, support20: indicators.support20, resistance20: indicators.resistance20, support60: indicators.support60, resistance60: indicators.resistance60, rules, zones: dynamicZones, reasons };
}
