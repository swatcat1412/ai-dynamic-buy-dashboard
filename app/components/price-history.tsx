...
const formatPrice = (value: number) => value >= 100 ? value.toFixed(0) : value.toFixed(2);
...
export default function PriceHistory() {
  ...
  const points = useMemo(() => {
    ...
    const x = (index: number) => bars.length === 1 ? (left + right) / 2 : left + (index / (bars.length - 1)) * width;
    const y = (value: number) => bottom - ((value - min) / spread) * (bottom - top);
    const path = (series: (number | null)[]) => series
      .map((value, index) => value === null ? "" : `${index ? "L" : "M"} ${x(index).toFixed(1)} ${y(value).toFixed(1)}`)
      .filter(Boolean)
      .join(" ");
    const grid = Array.from({ length: 5 }, (_, index) => {
      const ratio = index / 4;
      const value = max - ratio * spread;
      return { y: top + ratio * (bottom - top), value, label: formatPrice(value) };
    });
  ...
  return <div className="history-panel">
    <div className="history-toolbar">
      <div className={`live-state ${state}`}><i className="status-dot" />{state === "connected" ? `${bars.length} daily bars` : state === "loading" ? "Loading price history" : "History unavailable"}</div>
    ...
    {message ? <div className="indicator-message">{message}</div> : <div className="history-chart-wrap">
      <svg className="history-chart" viewBox="0 0 800 240" role="img" aria-label={`${symbol} closing price history with price scale`} preserveAspectRatio="none">
        <title>{symbol} price history</title>
        <desc>Closing price line with a 20-day exponential moving average and price labels.</desc>
        {points.grid.map((tick) => <g key={tick.label}><line x1="56" y1={tick.y} x2="780" y2={tick.y} className="chart-grid" /><text x="48" y={tick.y + 3} textAnchor="end" className="chart-price-label">{tick.label}</text></g>)}
        ...
        {points.ema && <path d={points.ema} className="chart-line ema-line" />}
      </svg>
      <div className="history-axis"><span>{points.firstDate || "—"}</span><span>Close <i className="chart-key close-key" /> EMA20 <i className="chart-key ema-key" /></span><span>{points.lastDate || "—"}</span></div>
      <div className="history-last">Latest close: <strong>{points.last === null ? "—" : points.last.toFixed(2)}</strong> · Range: {points.min ? `${points.min.toFixed(2)} – ${points.max.toFixed(2)}` : "—"}{!points.emaAvailable && bars.length > 0 ? " · EMA20 requires 20 days" : ""}</div>
    </div>}
  </div>;
}
