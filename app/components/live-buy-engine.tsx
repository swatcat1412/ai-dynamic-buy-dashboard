...
export default function LiveBuyEngine() {
  ...
  return <>
    ...
    {message ? <div className="indicator-message">{message}</div> : engine ? <div className="dynamic-engine-layout">
      <article className="engine-score-card"><div className="score-card-header"><span>{engine.symbol} buy engine</span><span className="score-date">DAILY</span></div><div className="engine-score">{engine.score}<span>/{engine.maximum}</span></div><p>Daily close <strong>${engine.price.toFixed(2)}</strong> · Zone <strong>{engine.currentZone}</strong></p><div className="entry-levels"><strong>Entry levels</strong>{engine.entryLevels.map((level) => <span key={level.label}>{level.label} <b>{level.price === null ? "—" : `${level.price.toFixed(2)}`}</b> · {level.allocation}</span>)}</div><div className="score-meter"><span style={{ width: `${engine.normalizedScore}%` }} /></div><div className={`engine-status ${engine.action.toLowerCase()}`}>{engine.action} · {engine.confidence}% data confidence</div><ul className="engine-reasons">{engine.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></article>
