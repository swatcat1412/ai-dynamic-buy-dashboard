...
import { allPortfolioSymbols, defaultPortfolioSymbol } from "../lib/portfolio-config";
...
export default function LiveBuyEngine() {
  ...
  return <>
    <div className="engine-toolbar"><span className={`live-state ${state}`}><i className="status-dot" />{state === "connected" ? "Stock opportunity connected" : state === "loading" ? "Calculating stock opportunity" : "Stock opportunity unavailable"}</span><label>Symbol <select value={symbol} onChange={(event) => { setState("loading"); setMessage(""); setSymbol(event.target.value); }}>{allPortfolioSymbols.map((item) => <option key={item}>{item}</option>)}</select></label></div>
