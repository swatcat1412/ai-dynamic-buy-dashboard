...
export default function LiveBuyZones() {
  ...
  useEffect(() => {
    ...
    Promise.all(symbols.map(async (symbol) => {
      const response = await fetch(`/api/market/buy-engine?symbol=${symbol}`, { cache: "no-store" });
      const payload = await response.json() as { ok?: boolean; engine?: Asset; error?: string };
      if (!response.ok || !payload.ok || !payload.engine) throw new Error(payload.error || `${symbol} data unavailable`);
      ...
      .catch((error: unknown) => {
        if (!cancelled) {
          setState("error");
          setMessage(error instanceof Error ? error.message : "Dynamic levels unavailable");
        }
      });
  ...
  return <div className="zone-grid">
    {assets.map((asset) => (
      <article className="zone-card" key={asset.symbol}>
        <div className="zone-card-header">
          <div>
            <span className="zone-symbol">{asset.symbol}</span>
            <p>Daily close {asset.price.toFixed(2)} · Current zone {asset.currentZone}</p>
          </div>
        ...
        <div className="zone-table" role="table" aria-label={`${asset.symbol} dynamic buy zones`}>
          ...
          {asset.zones.map((zone) => (
            <div className="zone-row" role="row" key={zone.zone}>
              <span className={`zone-badge ${zoneTone(zone.zone)}`}>{zone.zone}</span>
              <span className="threshold">{zone.range}</span>
              <span className={`action-label ${zoneTone(zone.zone)}`}>{zone.action}</span>
    ...
    {state === "loading" && assets.length === 0 ? <div className="indicator-message">กำลังคำนวณระดับราคาจากข้อมูลรายวัน...</div> : null}
  </div>;
}
